...
This is a tiny freestanding demo-ing using a rust style `main` in a `#[no_std]` free standing binary. Since this example uses a couple raw syscalls and assembly, it only works on x86-64 linux.
...
With the default `fn main() -> ()`, `cargo run` should print something like:

```text
In function: `__premain`
In function: `my_start`
